...
void UART_8(LPC_UART_TypeDef* UARTx, uint8_t Data){
	UART_SendByte((LPC_UART_TypeDef*)UARTx, Data);
}
void UART_32_DEC(LPC_UART_TypeDef* UARTx, uint32_t Data){
// This function uses recursion to convert decimal number
//   of unspecified length as an ASCII string
	if(Data >= 10){
		UART_32_DEC((LPC_UART_TypeDef*)UARTx, Data/10);
		Data = Data%10;
	}
//	UART_OutChar(n+'0'); // n is between 0 and 9
	UART_SendByte((LPC_UART_TypeDef*)UARTx, Data+'0');
}
/*
//---------------------UART_InUHex----------------------------------------
...
void UART_32_HEX(LPC_UART_TypeDef* UARTx, uint32_t Data){
	if(Data >= 0x10){
//		UART_OutUHex(number/0x10);
		UART_32_HEX((LPC_UART_TypeDef*)UARTx, Data/0x10);
//		UART_OutUHex(number%0x10);
		UART_32_HEX((LPC_UART_TypeDef*)UARTx, Data%0x10);
	}
	else{
...
//			UART_OutChar((number-0x0A)+'A');
	    	UART_SendByte((LPC_UART_TypeDef*)UARTx, ((Data-0x0A)+'A'));
	    }
	}
}
